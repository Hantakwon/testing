...
// 캐릭터 정보
let prevHp, prevAtk, prevSpeed, prevLevel, prevScore, prevExperience; // 상태 값 변화
let hp, atk, speed; // HP
let level = 1; // 레벨
...
function updateGameData() {
    if (hp !== prevHp || atk !== prevAtk || speed !== prevSpeed) {
        hpDiv.innerHTML = `HP : ${hp} <br> atk : ${atk} <br> speed : ${speed}`;
        prevHp = hp;
        prevAtk = atk;
        prevSpeed = speed;
    }
...
function showChoiceScreen() {
    ...
    choiceScreen.style.background = "rgba(0, 0, 0, 0.8)";
    choiceScreen.style.padding = "20px";
    choiceScreen.style.borderRadius = "10px";
    choiceScreen.style.color = "white";

    const options = [
        "Option 1: Increase Speed",
        "Option 2: Increase HP",
        "Option 3: Increase Attack"
    ];
